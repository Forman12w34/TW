...
console.log("Interface setup complete. Waiting 5 seconds before triggering the start button...");

setTimeout(() => {
    const startButton = $(".btn.evt-confirm-btn.btn-confirm-yes");

    if (!buttonClicked && startButton.length > 0) {
        console.log("Start button found, triggering click...");
        startButton.trigger("click"); 
        console.log("Start button clicked automatically after 5 seconds");

        buttonClicked = true;
    } else if (buttonClicked) {
        console.log("Start button click was already triggered.");
    } else {
...
async function createTable(list_launches, obj_stats, list_production, list_clusters_stats) {
    ...
    // Loop through the launches and create the table rows
    for (let i = 0; i < list_launches.length; i++) {
        ...
        html_prod_table += `
            <tr id="delete_row">
                <td>${i + 1}</td>
                <td><a href="${game_data.link_base_pure}info_village&id=${target_id}"><font color="${textColor}">${list_launches[i].name_destination}</font></a></td>
    ...
    // Hide wood, stone, iron for mobile if needed
    if (game_data.device != "desktop") {
        $(".hide_mobile").hide();
    }
    ...
    // Automatically trigger button clicks in sequence
    await triggerButtonClicksSequentially();
}

// Helper function to trigger the buttons sequentially with a delay
async function triggerButtonClicksSequentially() {
    const buttons = $(".btn_send");

    // Trigger each button click with a delay
